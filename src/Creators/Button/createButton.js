...
function createButton(props = {}) {
  ...
  bgRectangle.setAllCornerRadii(
    (["none", "full"].includes(roundness)
      ...
      : buttonProps.cornerRadius) || 0
  );
  bgRectangle.name = "BG";
  if (!shadow) bgRectangle.stroke = new Color(color);
  else
  ...
  let textColor = "#FFF";
  if (outlined || link) textColor = color;
  else textColor = tinyColor(color).isLight() ? "black" : "white";
